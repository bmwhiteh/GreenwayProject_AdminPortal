<?php
    ...
    $sqlClose = "UPDATE `maintenancetickets` SET `dtClosed` = '". $date ."', `strEmployeeAssigned` = NULL, `strEmployeeName` = NULL WHERE `intTicketId` = '".$ticketId."'";
    ...
    $sqlAddNote = "INSERT INTO `ticketnotes` (`intTicketId`, `strUserId`, `strEmployeeName`, `dateAdded`, `comment`) 
                    VALUES ('$ticketId', '$userId', '$userName', '$date', 'Ticket Closed.')";
    
    $resultAddNote = $conn->query($sqlAddNote) or die("Add note fail. $sqlAddNote");
    
     $sqlTask = "UPDATE `tasks` SET `lastCompleted`= '$dateReceived' WHERE `taskId`= '8'";
