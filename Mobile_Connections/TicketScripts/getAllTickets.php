<?php
    ...
    $ticketsSql = "SELECT `intTicketId`, `strTitle`, `tickettypes`.`strTicketType`, `strEmployeeAssigned`, `strEmployeeName`, `strDescription`, `dtSubmitted`, `bitUrgent`, `strImageFilePath`, `gpsLat`, `gpsLong` FROM `maintenancetickets` INNER JOIN  `tickettypes` ON  `maintenancetickets`.`intTypeId` =  `tickettypes`.`intTypeId` WHERE `dtClosed` is null";
    $ticketsResults = $conn->query($ticketsSql);
    $ticketsObj = array();
    while ($row = $ticketsResults->fetch_array(MYSQLI_ASSOC)) {
        $assignedName = $row['strEmployeeName'];
        ...
        $ticket->assigned = $row['strEmployeeAssigned'];
