<?php
    ...
    $ticketsSql = "SELECT `intTicketId`, `strTitle`, `tickettypes`.`strTicketType`, `strDescription`, `dtSubmitted`, `bitUrgent`, `strImageFilePath`, `gpsLat`, `gpsLong`, `dtClosed` FROM `maintenancetickets` INNER JOIN  `tickettypes` ON  `maintenancetickets`.`intTypeId` =  `tickettypes`.`intTypeId` WHERE `dtClosed` IS NOT NULL ORDER BY `dtClosed` DESC";
    ...
    while ($row = $ticketsResults->fetch_array(MYSQLI_ASSOC)) {
        ...
        $ticket->closedDate = $row['dtClosed'];
