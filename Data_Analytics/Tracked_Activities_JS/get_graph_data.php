...
if (is_ajax()) {
    ...
    //time spent on trails by month
        $sql = "SELECT sum(timeTotalDuration), month(startDate) FROM `userActivities` WHERE startDate >= $dtMinus1Year\n"
            . "GROUP BY Month(startDate)";
            
            
    //avg calories burned
        $sql = "SELECT avg(calTotalCalories), month(startDate) FROM `userActivities` WHERE startDate >= $dtMinus1Year\n"
            . "GROUP BY Month(startDate)";
    ...
    */
    ...
    if($graphType == 'pie_chart'){
        ...
        $sql = "SELECT count(intActivityId) as ActivityCount, strActivity as ActivityType FROM `userActivities`\n"
            . "LEFT JOIN userActivitiesType on userActivitiesType.intTypeId = userActivities.intActivityType\n"
            . " \n"
            . "WHERE startDate >= '$dtMinus1Year'\n"
            . " GROUP BY strActivity";
       /*avg dist traveled by month(Pie Chart)
        $sql = "SELECT avg(milesTotalDistance), month(startDate) FROM `userActivities` WHERE startDate >= $dtMinus1Year\n"
            . "GROUP BY Month(startDate)";
       */     
       ...
       //Tracked Activities by Day (Bar Chart)
        $sql = "SELECT count(intActivityId) as ActivityCount, strActivity as ActivityType, Weekday(startDate) as DayOfWeek\n"
            . "FROM `userActivities`\n"
            . "LEFT JOIN userActivitiesType on userActivitiesType.intTypeId = userActivities.intActivityType\n"
            . "WHERE startDate >= '$dtMinus1Year'\n"
       ...
       //Tracked Activities by Day (Bar Chart)
        ...
        $sql = "SELECT sum(milesTotalDistance) as totalDistance, month(startDate) as MonthAsInt FROM `userActivities` WHERE startDate >= $dtMinus1Year\n"
            . "GROUP BY Month(startDate)";
        ...
        $sql = "SELECT count(intActivityId) as ActivityCount, month(startDate) as ActivityDate, strActivity as ActivityType\n"
            . "FROM userActivities\n"
            . "LEFT JOIN userActivitiesType on userActivitiesType.intTypeId = userActivities.intActivityType\n"
            . "WHERE startDate >= '$dtMinus1Year'\n"
